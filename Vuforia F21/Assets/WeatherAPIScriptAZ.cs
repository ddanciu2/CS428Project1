...
public class WeatherAPIScriptAZ : MonoBehaviour
{
    public GameObject weatherTextObject;

    string url = "https://api.openweathermap.org/data/2.5/weather?lat=33.44&lon=-112.07&APPID=73a7bf2b3b9fe2a058317c2baaf31cd1&units=imperial";

    void Start()
    {

    // wait a couple seconds to start and then refresh every 10 minutes
        // Debug.Log("Picking random Number: " + PickRandomNumber());
    ...
    IEnumerator GetRequest(string uri)
    {
        using (UnityWebRequest webRequest = UnityWebRequest.Get(uri))
        {
            ...
            else
            {
                ...
                double convertFtoC = (tempF-32.0) * (5.0/9.0);

				int easyTempF = Mathf.RoundToInt((float)convertFtoC);
